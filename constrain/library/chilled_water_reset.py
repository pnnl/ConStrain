...
6.5.4.4 Chilled-Water Temperature Reset Controls
- Chilled-water systems with a design capacity exceeding 300,000 Btu/h supplying chilled or heated water to comfort conditioning systems shall include controls that automatically
...
- Code Year: 2016
- Code Section: 6.5.4.4 Chilled-Water Temperature Reset Controls

### Verification Approach

The verification checks chilled water temperature control under three conditions:
1. When outdoor temperature ≤ minimum threshold:
   - Chilled water temperature should be at maximum setpoint
2. When outdoor temperature ≥ maximum threshold:
   - Chilled water temperature should be at minimum setpoint
3. When outdoor temperature is between thresholds:
   - Chilled water temperature should modulate between min/max setpoints
Note: Verification is bypassed when there is no chilled water flow.
...
- Climate Zone(s): any
- Component(s): chilled water temperature sensors, outdoor air sensors

### Verification Algorithm Pseudo Code

```python
if flow_mass_water_chilled <= 0:
    pass  # No flow condition
elif temperature_air_outdoor <= temperature_air_outdoor_min:
    if temperature_water_chilled >= temperature_water_chilled_setpoint_min * 0.99:
        pass  # Proper reset at low outdoor temp
    else:
        fail
elif temperature_air_outdoor >= temperature_air_outdoor_max:
    if temperature_water_chilled <= temperature_water_chilled_setpoint_max * 1.01:
    ...
    if temperature_water_chilled_setpoint_min * 0.99 <= temperature_water_chilled <= temperature_water_chilled_setpoint_max * 1.01:
...
- temperature_water_chilled: Chilled water temperature
  - Data Value Unit: °C
  - Data Point Affiliation: System monitoring

- flow_mass_water_chilled: Chilled water flow
  - Data Value Unit: mass flow rate
  - Data Point Affiliation: System monitoring

- temperature_water_chilled_setpoint_max: Maximum temperature setpoint
  - Data Value Unit: °C
  - Data Point Affiliation: System control

- temperature_water_chilled_setpoint_min: Minimum temperature setpoint
  - Data Value Unit: °C
  - Data Point Affiliation: System control
...
class CHWReset(RuleCheckBase):
    ...
    def verify(self):
        self.result = (
            (
                self.df["flow_mass_water_chilled"]
                <= self.get_tolerance("waterflow", "chilled")
            )
            | (
                (
                ...
                & (
                    self.df["temperature_water_chilled"]
                    >= self.df["temperature_water_chilled_setpoint_min"]
            ...
            | (
                ...
                & (
                    self.df["temperature_water_chilled"]
                    <= self.df["temperature_water_chilled_setpoint_max"]
                    + self.get_tolerance("temperature", "general")
                )
            )
            | (
    ...
    def plot(self, plot_option, fig_size, plt_pts=None):
        ...
        plt.subplots()
        sns.scatterplot(
            x="temperature_air_outdoor", y="temperature_water_chilled", data=self.df
        )
