...
class ZoneTempControl(RuleCheckBase):
    ...
    def verify(self):
        self.result = (self.df["T_cool_set"] - self.df["T_heat_set"]) > 2.77
...
class GuestRoomControlTemp(RuleCheckBase):
    points = ["T_heat_set", "T_cool_set", "O_sch", "tol_occ", "tol_temp"]
        ...
        for idx, day in self.df.groupby(self.df.index.date):
            if (
                ...
                if (
                    ...
                    if (day["T_heat_set"] < 15.6 + tol_temp).all() and (
                        day["T_cool_set"] > 26.7 - tol_temp
                    ).all():
                        result_repo.append(
                            1
                    ...
                    if (
                        day["T_heat_set"] < T_z_hea_occ_set - 2.22 + tol_temp
                    ).all() or (
                        day["T_cool_set"] > T_z_coo_occ_set + 2.22 - tol_temp
                    ).all():
                        result_repo.append(
                            1
