from PyQt6.QtWidgets import QLabel, QLineEdit, QVBoxLayout, QPushButton, QHBoxLayout, QComboBox, QListWidget, QDialog, QGroupBox, QDialogButtonBox, QMenu, QMessageBox
...
class PopupWindow(QDialog):
    ...
    def initialize_ui(self):
        ...
        self.method_combo_box.currentIndexChanged.connect(lambda: self.update_form(False))
    ...
    def update_list(self, list_widget):
        ...
        if list_widget == self.parameter_list_widget:
            ...
            for input in to_add:
                list_widget.addItem(f'{input[0]}({input[1]}) equals {input[2]}: {input[3]}')
    ...
    def save(self):
        ...
        object_type = self.object_type_combo_box.currentText()
        method = self.method_combo_box.currentText()
        capitalized_keys = ['Type', 'MethodCall', 'Parameters', 'Payloads', 'Next', 'Choices', 'Default', 'Title']
        ...
        if self.current_choices is not None and len(self.current_choices) > 0:
            ...
            for c in self.current_choices:
                if len(c) == 4:
                    ...
                    self.form_data['Choices'].append({'Value': value, 'Equals': c[2], 'Next': c[3]})
                elif len(c) == 3:
                    self.form_data['Choices'].append({'Value': c[0], 'Equals': c[1], 'Next': c[2]})
