from PyQt6.QtWidgets import QVBoxLayout, QWidget, QPushButton, QGraphicsView, QGraphicsTextItem
...
class Zoom(QGraphicsView):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton and event.pos() == self.dragStartPosition:
            item = self.itemAt(event.pos())
            if item:
                if isinstance(item, QGraphicsTextItem):
                    item = item.parentItem()
...
class WorkflowDiagram(QWidget):
    ...
    def create_item(self, state):
        ...
        if 'Next' in state.keys():
            matching_rects = [rect for rect in rects if rect.state['Title'] == state['Next']]
