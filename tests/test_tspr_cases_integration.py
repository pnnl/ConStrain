...
class TestTSPRCasesIntegration(unittest.TestCase):
    ...
    def test_tspr_cases_script_generates_passing_report(self):
        ...
        for i, row in enumerate(data_rows):
            ...
            try:
                ...
                self.fail(
                    f"Case {case_no} ({verification_class}): Sample count '{sample_count}' is not a valid integer"
                )
    ...
    def test_report_format_validation(self):
        ...
        self.assertIn(
            "| Case No.", report_content, "Report should contain the table header"
        )
        ...
        for column in expected_columns:
            self.assertIn(
                column, report_content, f"Report should contain column '{column}'"
            )
